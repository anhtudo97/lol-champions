...
let modalSearchBlock = new bootstrap.Modal(
  document.getElementById("editBlockModal")
);
let modalEditBlock = new bootstrap.Modal(
  document.getElementById("editBlockModalDetail")
);
let blockSelect;
...
let renderDataTable = (parentEl, data) => {
  let html = data
    .map((champion, index) => {
      return `<tr data-key="${champion.key}"><th scope="row" class="bg-light">${
        index + 2
      }</th><td class="disabled"><div class="champion-meta"><div class="champion-img"><img src="${
        champion.icon
      }"/></div> ${champion.name}</div></td><td id="b${
        index + 2
      }" data-bs-toggle="tooltip" data-bs-placement="right" title="B : ${
        index + 2
      }">${champion.stats.hp}</td><td id="c${
        index + 2
      }" data-bs-toggle="tooltip" data-bs-placement="right" title="C : ${
        index + 2
      }">${champion.stats.mp}</td><td id="d${
        index + 2
      }"  data-bs-toggle="tooltip" data-bs-placement="right" title="D : ${
        index + 2
      }">${champion.stats.attackdamage}</td><td id="e${
        index + 2
      }" data-bs-toggle="tooltip" data-bs-placement="right" title="E : ${
        index + 2
      }">${champion.stats.attackspeed}</td><td id="f${
        index + 2
      }" data-bs-toggle="tooltip" data-bs-placement="right" title="F : ${
        index + 2
      }">${champion.stats.attackrange}</td></tr>`;
    })
    .join("");
...
let isValidColName = (colName, tableEl) => {
  return (
    getTableTheadNames(tableEl).includes(colName.toLowerCase()) &&
    colName.toLowerCase() !== "a"
  );
};
...
let isValidRowValue = (rowValue, tableContainerEl) => {
  return (
    Number(rowValue) >= 2 &&
    Number(rowValue) <= tableContainerEl.childElementCount
  );
};
...
btnEdit.onclick = () => {
  if (
    isValidColName(inputCol.value, table) &&
    isValidRowValue(inputRow.value, tableContainer)
  ) {
    blockSelect = document.querySelector(
      "#" + inputCol.value.toLowerCase() + inputRow.value
    );
    clearError(editForm);
    modalSearchBlock.hide();
    modalEditBlock.show();
    editDetailForm.querySelector(
      ".modal-title"
    ).innerText = `Sửa dữ liệu ô [${inputCol.value.toUpperCase()}:${
      inputRow.value
    }] - ${blockSelect.innerText}`;
  } else {
